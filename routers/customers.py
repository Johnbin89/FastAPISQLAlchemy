...
from ..models.models import SessionLocal, Customers, Items, Orders, so, sa
...
def create_customer(customer: schemas.Customer, db: so.Session = Depends(get_db)):
...
def read_customers(skip: int = 0, limit: int = 100, db: so.Session = Depends(get_db)):
    return db.query(Customers).offset(skip).limit(limit).all()


@router.get("/customers/{customer_id}", response_model=schemas.Customer)
def read_customer(customer_id: int, db: so.Session = Depends(get_db)):
...
def read_customer_orders(cust_id: int, db: so.Session = Depends(get_db)):
